...
async function checkUserCalibration() {
    try {
        const response = await fetch(window.apiEndpoints?.user_check || '/api/user/check', {
            method: 'POST',
...
async function loadPersonalCalibration() {
    try {
        const response = await fetch(`/api/calibration/user/${window.userId}`);
        if (!response.ok) {
...
async function updateUserCalibrationStatus(hasCalibration = true) {
    try {
        const response = await fetch(window.apiEndpoints?.user_update || '/api/user/update-calibration', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                user_id: window.userId,
...
async function sendCalibrationData(calibrationData) {
    try {
        // 1. 캘리브레이션 데이터 저장
        const response = await fetch(window.apiEndpoints?.calibration || '/api/calibration', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                user_id: window.userId,
