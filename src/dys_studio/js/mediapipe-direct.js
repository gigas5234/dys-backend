...
async function initializeExpressionAnalyzer() {
    ...
    try {
        // 서버에 표정 분석기 초기화 요청
        const response = await fetch(`${window.serverUrl || 'https://34.64.136.237'}/api/expression/initialize`, {
            method: 'POST',
            headers: {
...
async function analyzeExpressionFromVideo(videoEl) {
    try {
        ...
        // 서버에 표정 분석 요청
        const response = await fetch(`${window.serverUrl || 'https://34.64.136.237'}/api/expression/analyze`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
